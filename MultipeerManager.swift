...
extension MultipeerManager: MCSessionDelegate, MCNearbyServiceAdvertiserDelegate, MCNearbyServiceBrowserDelegate {
    ...
    func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID) {
        // Clever filtering: JSON and Network commands are valid UTF-8 strings.
        // Compressed MP4/MP3 audio chunks are not valid UTF-8 and will fail this check.
        if let text = String(data: data, encoding: .utf8) {
            DispatchQueue.main.async {
                let handled = WebRTCManager.shared.handleCommand(text)
                if !handled {
                    self.processReceivedData(data)
                }
            }
        } else {
            // It's a binary audio stream chunk sent over Multipeer Wi-Fi! Append it directly to the buffer.
            DispatchQueue.main.async {
